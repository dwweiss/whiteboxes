...
class GasMix(Gas):
    ...
    Mole fraction or volumetric fraction y_i ([kmol/kmol] or %vol):
        N = m / M
    ...
    Example:
        ...
        heat capacity of humid air = dry air + water vapor:
            ...
            c_p = c_air * m_air/m_tot + c_vap * m_vap/m_tot
                = c_air * x_air * M_air/M_tot + c_vap * x_vap * M_vap/M_tot
    ...
    def __init__(self, identifier: str = 'gas_mix',
        ...
        self._lambda_mix_formula: str = 'mason'   # ['mason', 'mole']
    ...
    def check(self, silent: bool = False) -> bool:
        ...
        if not ok and not silent:
            print('!!! sum of mole fractions is not one', sum_mole_frac, 
                  self.components.values())
        return ok

    def _c_p(self, T: float, p: float = 1e5, x: float = 0.) -> float:
        x_all, M_all, c_p_all = [], [], []
        ...
        for gas, mole_frac in self.components.items():
            ...
            x_all.append(mole_frac) 
            M_all.append(gas.M) 
            c_p_all.append(gas.c_p(T, p_partial))
            
        return mix_mass(x=x_all, M=M_all, property_=c_p_all)

    def _lambda(self, T: float, p: float = 1e5, x: float = 0.) -> float:
        x_all, M_all, lambda_all, mu_all = [], [], [], []
        ...
        for gas, mole_frac in self.components.items():
            ...
            x_all.append(mole_frac) 
            M_all.append(gas.M) 
            lambda_all.append(gas.lambda_(T, p_partial)) 
            mu_all.append(gas.mu(T, p_partial))

        if self._lambda_mix_formula.startswith('mas'):
            lambda_ = mix_mason(x=x_all, M=M_all, lambda_=lambda_all, 
                                mu=mu_all)
        else:
            lambda_ = mix_mole(x=x_all, M=M_all, property_=lambda_all)
            
        return lambda_

    def _mu(self, T: float, p: float = 1e5, x: float = 0.) -> float:
        x_all, M_all, mu_all = [], [], []
        ...
        return mix_mole(x=x_all, M=M_all, property_=mu_all)

    def _rho(self, T: float, p: float = 1e5, x: float = 0.) -> float:
        rho = 0.
        for gas, mole_frac in self.components.items():
            if mole_frac < 1e-8:
                continue
