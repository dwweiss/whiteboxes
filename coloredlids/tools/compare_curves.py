...
class CompareCurves(Base):
    ...
    def load(self) -> bool:
        ...
        if self.gui:
            self.filenames = _dialog_load_filenames(min_files=self.curves)
...
def _dialog_load_filenames(default_ext: str='data', 
                           initial_dir: Optional[str]=None,
                           min_files: int=1) -> List[str]:
    """
    Asks for file names to be loaded, employing tkinter interface
